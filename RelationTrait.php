...
trait RelationTrait
{
    public function loadAll($POST, $allowedRelations = [])
    {
        if ($this->load($POST)) {
    ...
    public function saveAll($allowedRelations = [])
    {
        /* @var $this ActiveRecord */
        $db = $this->getDb();
            ...
            if ($this->save()) {
                ...
                if (!empty($this->relatedRecords)) {
                    foreach ($this->relatedRecords as $name => $records) {
                        ...
                        if (!in_array($name, $allowedRelations))
                            continue;
                ...
                foreach ($noChildren as $relName) {
                    ...
                    if (empty($relData[$relName]['via'])) {
                        ...
                        $condition = [];
                        $isManyMany = count($relModel->primaryKey());
                        if ($isManyMany) {
    ...
    public function deleteWithRelated($allowedRelations)
    {
        ...
        try {
            ...
            foreach ($relData as $data) {
                ...
                if (!in_array($data['name'], $allowedRelations))
                    continue;
    ...
    public function getRelationData()
    {
        ...
        foreach ($reflection->getMethods() as $method) {
            ...
            try {
                ...
                //if method return type is invalid,
            }
        }
        return $stack;
    }
